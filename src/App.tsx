...
function AppContent() {
  ...
  }, [setPlaylistProgress]); // Run ONLY once on mount
  ...
  return (
    <div className="h-screen w-screen bg-[#0a0f1c] text-slate-100 font-sans selection:bg-purple-500/30 flex flex-col items-center overflow-hidden relative">
      ...
      {dragActive && (
        <div
          className={`fixed inset-0 z-[1000000] bg-[#0a0f1c]/80 backdrop-blur-md border-4 border-dashed ${isValidDrag ? "border-purple-500 cursor-copy" : "border-red-500 cursor-not-allowed"} flex items-center justify-center transition-all`}
        >
          <div className="text-center">
            <div
              className={`w-24 h-24 mx-auto rounded-full ${isValidDrag ? "bg-purple-500/20 border-purple-500/30 shadow-[0_0_50px_rgba(168,85,247,0.4)]" : "bg-red-500/20 border-red-500/30 shadow-[0_0_50px_rgba(239,68,68,0.4)]"} flex items-center justify-center mb-6 border`}
            >
              <UploadCloud
                className={`w-12 h-12 ${isValidDrag ? "text-purple-400" : "text-red-400"}`}
              />
            </div>
            ...
            <p
              className={`${isValidDrag ? "text-purple-300" : "text-red-300"} mt-2 text-lg px-8`}
            >
