...
const TruncatedText = ({
  ...
  return (
    <div className={`group relative ${wrapperClassName || "w-full"}`}>
      <div ref={textRef} className={`${className} truncate max-w-full`}>
        {text}
      </div>
      {isOverflowing && (
        <div
          ...
          {history.length > 0 && (
            <div className="flex items-center bg-[#111728] px-4 py-2 rounded-2xl border border-white/5 shadow-2xl shrink h-[46px] relative z-[100] w-full max-w-[280px] min-w-[140px] group transition-colors focus-within:border-purple-500/50">
              ...
              <input
                ...
                placeholder={
                  t.history.searchPlaceholder || "Search by title..."
                }
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative inline-flex items-center">
                <input
                  ...
                  onChange={() =>
                    setDeleteFilesOnHistoryDelete(!deleteFilesOnHistoryDelete)
                  }
                />
          ...
          <table className="w-full text-left border-separate border-spacing-0 relative table-fixed min-w-[1000px]">
            <thead className="shadow-md border-b border-white/10">
              <tr className="text-[10px] uppercase tracking-widest text-slate-400">
                ...
                <th className="px-8 py-5 font-semibold w-[30%] min-w-[280px] bg-[#141b2e]/95 backdrop-blur-md text-left">
                  {t.history.tableTrack}
                </th>
                <th className="px-8 py-5 font-semibold w-[15%] min-w-[150px] bg-[#141b2e]/95 backdrop-blur-md text-left">
                  {t.history.tableDate}
                </th>
                <th className="px-4 py-5 font-semibold text-center w-[11%] min-w-[110px] bg-[#141b2e]/95 backdrop-blur-md">
                  {t.history.tableBpm}
                </th>
                <th className="px-4 py-5 font-semibold text-center w-[11%] min-w-[110px] bg-[#141b2e]/95 backdrop-blur-md">
                  {t.history.tableKey}
                </th>
                <th className="px-4 py-5 font-semibold text-center w-[28%] min-w-[300px] rounded-tr-3xl bg-[#141b2e]/95 backdrop-blur-md">
                  {t.history.tableAction}
                </th>
              </tr>
            </thead>
            <tbody>
              {filteredHistory.length === 0 ? (
                <tr>
                  <td
                    colSpan={6}
                    className="px-8 py-24 text-center text-slate-500"
                  >
                    ...
                    <p className="mt-1">
                      {history.length === 0 ? t.history.emptyDesc : ""}
                    </p>
                  </td>
                </tr>
              ) : (
                displayedHistory.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      <td
                        className={`pl-10 pr-6 py-5 text-center transition-colors duration-300 group-hover/row:bg-[#141b2e]/30 ${isLastRow ? "rounded-bl-3xl" : ""}`}
                      >
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-slate-900/95 backdrop-blur-md border border-white/10 text-white text-[10px] rounded-xl opacity-0 group-hover/status:opacity-100 transition-all duration-300 delay-0 group-hover/status:delay-500 pointer-events-none whitespace-nowrap z-[100] shadow-2xl scale-95 group-hover/status:scale-100 font-bold uppercase tracking-wider">
                            {(() => {
                              ...
                              if (
                                ...
                                return t.history.sources.youtube;
                              if (url.includes("soundcloud.com"))
                                return t.history.sources.soundcloud;
                      ...
                      <td
                        className={`px-2 py-5 text-center transition-colors duration-300 group-hover/row:bg-[#141b2e]/30`}
                      >
                      ...
                      <td
                        className={`px-4 py-5 text-center transition-colors duration-300 group-hover/row:bg-[#141b2e]/30 ${isLastRow ? "rounded-br-3xl" : ""}`}
                      >
                            ...
                            item.url ? (
                              ...
                              <div className="group relative">
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleDownloadFromHistory(item)
                                  }
